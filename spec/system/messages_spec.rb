...
RSpec.describe "メッセージ投稿機能", type: :system do
  ...
  context '投稿に成功したとき' do
    it 'テキストの投稿に成功すると、投稿一覧に遷移して、投稿した内容が表示されている' do
      ...
      # 値をテキストフォームに入力する
      post = "テスト"
      fill_in 'message_current', with: post
      # 送信した値がDBに保存されていることを確認する
      expect {
        find('input[name="commit"]').click_on
      }.to change { Message.count }.by(1)
      # 投稿一覧画面に遷移していることを確認する
      edxpect(current_path)to. eq room_messages_path(@room_user.room)
    ...
    it '画像の投稿に成功すると、投稿一覧に遷移して、投稿した画像が表示されている' do
      ...
      # 投稿一覧画面に遷移していることを確認する
       expect(current_path)to. eq room_messages_path(@room.name)
    ...
    it 'テキストと画像の投稿に成功すること' do
      ...
      # 送信した値がDBに保存されていることを確認する
       expect{
         find('input[name="commit"]').clicl_on
       }to.change { Message.count }.by(1)
      # 送信した値がブラウザに表示されていることを確認する
      expect(page).to have_content(post)
      # 送信した画像がブラウザに表示されていることを確認する
